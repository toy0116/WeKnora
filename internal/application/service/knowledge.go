...
func (s *knowledgeService) CreateKnowledgeFromFile(ctx context.Context,
	...
	task := asynq.NewTask(types.TypeDocumentProcess, payloadBytes, asynq.Queue("default"), asynq.MaxRetry(3), asynq.Timeout(3*time.Hour))
...
func (s *knowledgeService) createKnowledgeFromFileURL(
	...
	task := asynq.NewTask(types.TypeDocumentProcess, payloadBytes, asynq.Queue("default"), asynq.Timeout(3*time.Hour))
	info, err := s.task.Enqueue(task)
	if err != nil {
...
func (s *knowledgeService) getSummary(ctx context.Context,
	...
	// LocalHub patch (2026-05-01) · 让 summary 跟原文语言走 · 不跟 user-locale。
	// 跟 wiki-source-language patch 同模式 · 用前 3 个 chunk 内容检测 · 覆盖 ctx。
	// 修了之后 LanguageNameFromContext(ctx) 会读到 detected 值 · prompt 用对的语言。
	{
		sample := ""
		for i, ch := range sortedChunks {
			if i >= 3 {
				break
			}
			sample += ch.Content + "\n"
		}
		detected := secutils.DetectChunkLanguage(sample)
		currentLang, _ := types.LanguageFromContext(ctx)
		if detected != "" && detected != currentLang {
			logger.Infof(ctx, "summary: doc %s language override op=%q → detected=%q (LocalHub patch)",
				knowledge.ID, currentLang, detected)
			ctx = context.WithValue(ctx, types.LanguageContextKey, detected)
		}
	}
...
func (s *knowledgeService) enqueueImageMultimodalTasks(
	...
	for _, img := range images {
		...
		task := asynq.NewTask(types.TypeImageMultimodal, payloadBytes, asynq.Queue("critical"), asynq.MaxRetry(3))
		if _, err := s.task.Enqueue(task); err != nil {
