...
// Ported from upstream 9ad8e7ca (was blocked on Tier 3 1f5970b6, now landed).
...
// TestBuildAssistantHistoryMessages_ForwardsReasoningContentForMimo guards
// the integration point between upstream's DB-rebuild path (1f5970b6) and
// our MiMo / DeepSeek-thinking-mode plumbing (73cbfc35 + 6db4aaf4).
//
// MiMo rejects multi-turn requests where the prior assistant+tool_calls
// message lacks reasoning_content (HTTP 400). The forward path requires
// AgentStep.ReasoningContent (persisted in Message.AgentSteps jsonb,
// json-tagged "thinking_content" for backward compat) to land on the
// rebuilt chat.Message — otherwise the chat_provider_spec.go sanitizer
// drops the entire turn and MiMo loses context.
//
// Without this regression test, anyone refactoring buildAssistantHistoryMessages
// in the future could quietly drop the field assignment and regress us
// to "MiMo HTTP 400 on every multi-turn agent reply" silently.
func TestBuildAssistantHistoryMessages_ForwardsReasoningContentForMimo(t *testing.T) {
	const reasoning = "The user wants the spec, so I'll first call KB search to find the datasheet, then compose the summary from those chunks."
	msg := &types.Message{
		Role:    "assistant",
		Content: "Here is the R1520LG hardware spec...",
		AgentSteps: types.AgentSteps{
			{
				Iteration:        0,
				Thought:          "Let me search the KB.",
				ReasoningContent: reasoning,
				ToolCalls: []types.ToolCall{
					{
						ID:   "call_kb",
						Name: agenttools.ToolKnowledgeSearch,
						Args: map[string]interface{}{"query": "R1520LG"},
						Result: &types.ToolResult{
							Success: true,
							Output:  "datasheet chunks",
						},
					},
				},
			},
	...
	got := buildAssistantHistoryMessages(msg)
	if !assert.GreaterOrEqual(t, len(got), 1) {
		return
	}
	// The first message is the assistant+tool_calls turn — that's the one
	// MiMo will reject if ReasoningContent is missing.
	assert.Equal(t, "assistant", got[0].Role)
	assert.NotEmpty(t, got[0].ToolCalls, "expected tool_calls on the replayed assistant turn")
	assert.Equal(t, reasoning, got[0].ReasoningContent,
		"AgentStep.ReasoningContent must be forwarded to chat.Message.ReasoningContent "+
			"so MiMo / DeepSeek thinking-mode multi-turn requests don't 400")
}
