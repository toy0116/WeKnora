...
func buildUserHistoryMessage(m *types.Message) chat.Message {
	...
	// Cherry-picked from upstream 9ad8e7ca (was blocked on the Tier 3 refactor
	// 1f5970b6 — now unblocked).
...
func buildAssistantHistoryMessages(m *types.Message) []chat.Message {
	...
	for _, step := range m.AgentSteps {
		...
		assistantMsg := chat.Message{
			...
			// MiMo + DeepSeek V3.2+ thinking-mode reject multi-turn requests
			// where the prior assistant+tool_calls message lacks its
			// reasoning_content (HTTP 400). The full plumbing lives in our
			// 73cbfc35 + 6db4aaf4 fix; here we restore it across the
			// 1f5970b6 DB-rebuild path: AgentStep.ReasoningContent is
			// persisted in the Message.AgentSteps jsonb column (json tag
			// "thinking_content" preserved for backward compat) and gets
			// forwarded onto the rebuilt assistant message so MiMo's
			// sanitizer in chat_provider_spec.go doesn't drop the turn.
