...
import (
	"context"
	"fmt"
	"log" // LocalHub patch · skip-and-log undecodable task messages (rdb.ListLeaseExpired / read aggregation set)
	"math"
	"sync"
	"time"
...
func (r *RDB) ReadAggregationSet(qname, gname, setID string) ([]*base.TaskMessage, time.Time, error) {
	...
	for _, s := range data {
		...
		if err != nil {
			// LocalHub patch (同上 · ListLeaseExpired 的姊妹点 · aggregation set decode)
...
func (r *RDB) ListLeaseExpired(cutoff time.Time, qnames ...string) ([]*base.TaskMessage, error) {
	...
	for _, qname := range qnames {
		...
		for _, s := range data {
			msg, err := base.DecodeMessage([]byte(s))
			if err != nil {
				// LocalHub patch (TY review · 2026-05-28):
				//   原代码: 一条坏 message → 整批 abort · recoverer 永远跑不完 · worker 卡死
				//   场景: Redis 里某条 task 的 protobuf wire-format 坏了 (crash / 版本不匹配 / GC 半途)
				//   修复: 跳过坏 message + log 警告 · 不让单点污染拖垮整个 queue
				//   注: 上游 asynq v0.26.0 还没修这 4 处 (ListLeaseExpired / ListGroupTasks / listGroupTasks)
				//        · 这是 vendor patch · 待 upstream PR (hibiken/asynq)
